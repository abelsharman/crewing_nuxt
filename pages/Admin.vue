    ...
    <div v-if="!loading" style="padding-top: 20vh; padding-left: 4vw">
      <div class="vac_list">
        ...
        >
          <div class="vac_list_item_main">
            ...
            <p>{{ item.text }}</p>
            <v-btn depressed color="primary" @click="deleteSpec(item.id)">
              Удалить
            </v-btn>
          </div>
      ...
      <div class="vac_form">
        <div class="vac_list_item">
          <div class="vac_list_item_main">
            <h1 style="margin: 0; width: 100%; position: absolute">
              Форма для заполнения новой должности
            </h1>
            <v-text-field
            ...
            <v-btn depressed color="primary" @click="addSpec">
              Добавить должность
            </v-btn>
          </div>
          <div class="vac_list_item_dop">
            <p class="vac_list_item_dop_1">Судно</p>
            <p class="vac_list_item_dop_2">
              <v-text-field
...
export default {
  ...
  methods: {
    ...
    deleteSpec(id) {
      ...
      this.$axios
        .delete(`https://crewing.kz/crewingsDelete/${id}`, config)
        .then((res) => {
          console.log(res);
          this.getVac();
