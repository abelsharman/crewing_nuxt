<template>
  <div
    ...
    :style="[
      isMain && width < 490 ? { 'background-color': 'transparent' } : {},
    ]"
  >
    <a href="/" class="header_img"
      ><img
        :class="{ header_logo_scroll: !isMain }"
    ...
    <a class="header_tel" :href="'tel:' + phone" v-if="phone.length > 0">{{
      phone
    }}</a>

    <span
      id="header_123"
      style="position: absolute; bottom: -20px; left: 50vw; opacity: 0"
      >a</span
    >
    <v-app-bar
      :style="[
        !isMain ? { background: 'white' } : { background: 'transparent' },
      ]"
    ...
    >
      <a href="/"
        ><img
          class="header_mobile_img"
          :class="{ header_logo_scroll: !isMain }"
          :src="getImgUrl()"
          alt=""
      /></a>
      <p :style="[!isMain ? { color: '#173760' } : { color: '#173760' }]">
        Marine Personnel & Consulting
      </p>
      <v-app-bar-nav-icon
        large
        @click.stop="drawer = !drawer"
        style="margin-left: 30vw; margin-top: 24px"
        :style="[isMain ? { color: '#173760' } : { color: '#173760' }]"
      ></v-app-bar-nav-icon>
    </v-app-bar>
      ...
      <v-list nav dense>
        ...
        >
          <v-list-item>
            <v-list-item-title
              ><a
                style="color: white; text-decoration: none; font-size: 4vw"
                class="header_mobile_a"
                href="/"
                >Главная</a
              ></v-list-item-title
            >
